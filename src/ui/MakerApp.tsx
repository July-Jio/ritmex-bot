...
export function MakerApp({ onExit }: MakerAppProps) {
  const [snapshots, setSnapshots] = useState<Record<string, MakerEngineSnapshot>>({});
  const [error, setError] = useState<Error | null>(null);
  const enginesRef = useRef<Record<string, MakerEngine>>({});
  ...
  useEffect(() => {
    ...
    try {
      const newEngines: Record<string, MakerEngine> = {};
      const initial: Record<string, MakerEngineSnapshot> = {};
      accounts.forEach((acc, idx) => {
        ...
        const engine = new MakerEngine({ ...makerConfig, symbol }, adapter);
  ...
  const totals = entries.reduce(
    (acc, [, snap]) => {
      ...
      acc.totalTrades += snap.tradeStats.totalTrades;
  ...
  const renderPanel = (label: string, snapshot: MakerEngineSnapshot) => {
    ...
    return (
      <Box flexDirection="column" marginBottom={1} borderStyle="round" paddingX={1} key={label}>
        <Text color="cyanBright">{label} ｜ {snapshot.symbol}</Text>
        <Text>
          买一价: {formatNumber(topBid, 2)} ｜ 卖一价: {formatNumber(topAsk, 2)} ｜ 点差: {spreadDisplay}
        </Text>
        <Text color="gray">状态: {snapshot.ready ? "实时运行" : "等待市场数据"}</Text>
        <Text>
          账户成交额: {formatNumber(snapshot.sessionVolume, 2)} USDT ｜ 交易次数: {snapshot.tradeStats.totalTrades} ｜ 手续费: {formatNumber(snapshot.totalFees, 4)} USDT
        </Text>
        <Text>
          交易量/分钟: {formatNumber(snapshot.volumeMetrics.volumePerMinute, 2)} USDT ｜ 交易量/小时: {formatNumber(snapshot.volumeMetrics.volumePerHour, 2)} USDT ｜ 成功率: {formatNumber(snapshot.volumeMetrics.successRate, 1)}%
        </Text>
        <Text>
          账户权益: {formatNumber(snapshot.accountBalance, 2)} USDT ｜ 未实现盈亏: {formatNumber(snapshot.accountUnrealized, 4)} USDT ｜ 净盈亏: {formatNumber(snapshot.netPnl, 4)} USDT
        </Text>
        <Text color="yellow">
        ...
        <Box flexDirection="column" marginTop={1}>
          ...
          {openOrderRows.length > 0 ? <DataTable columns={openOrderColumns} rows={openOrderRows} /> : <Text color="gray">暂无挂单</Text>}
        </Box>
        <Box flexDirection="column" marginTop={1}>
          <Text color="yellow">最近事件</Text>
          {lastLogs.length > 0 ? lastLogs.map((item, index) => (
            <Text key={`${item.time}-${index}`}>[{item.time}] [{item.type}] {item.detail}</Text>
  ...
  return (
    <Box flexDirection="column" paddingX={1}>
      <Box flexDirection="column" marginBottom={1}>
        <Text color="cyanBright">Maker Strategy Dashboard（多账户）</Text>
        <Text color="gray">按 Esc 返回策略选择</Text>
        <Text>
          总成交额: {formatNumber(totals.volume, 2)} USDT ｜ 总交易次数: {totals.totalTrades} ｜ 总手续费: {formatNumber(totals.totalFees, 4)} USDT
        </Text>
        <Text>
